...
try {
    ...
    $private:releaseArgs = @{
        ...
        TargetCommitish = if (git rev-parse --show-superproject-working-tree) { git --git-dir "$(git rev-parse --show-superproject-working-tree)/.git" rev-parse $env:RELEASE_TAG_REF }
