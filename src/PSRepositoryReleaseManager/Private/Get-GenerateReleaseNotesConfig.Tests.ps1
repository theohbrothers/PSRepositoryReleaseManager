...
Describe "Get-GenerateReleaseNotesConfig" -Tag 'Unit' {
    ...
    AfterEach {
        Remove-Item $repo
        Remove-Item env:PROJECT_DIRECTORY
        Remove-Item env:RELEASE_TAG_REF
        Remove-Item env:RELEASE_NOTES_VARIANT
        Remove-Item env:RELEASE_NOTES_PATH
    }
    ...
    It 'Gets config using env vars' {
        ...
        $c['TagName'] | Should -Be $env:RELEASE_TAG_REF
